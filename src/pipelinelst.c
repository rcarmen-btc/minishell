...
int	get_cmd_args_cnt(t_lst *tokenlst)
{
	...
	cmd_args_cnt = 0;
	while (tokenlst && (tokenlst->type == TOKEN_CMD || \
...
t_lst *pipeline_tmp, int cmd_index, int *prev_pos)
{
		...
		is_str_token((*tokenlst)->type) && *prev_pos == ARG_IN_ONE_WITH_NEXT)
	{
		*prev_pos = (*tokenlst)->str_position;
		...
		ft_strlcat(pipeline_tmp->cmd[cmd_index], (*tokenlst)->value, \
				ft_strlen((*tokenlst)->value) + \
				ft_strlen(pipeline_tmp->cmd[cmd_index]) + 1);
		if ((*tokenlst)->type == TOKEN_CMD || is_str_token((*tokenlst)->type))
			(*tokenlst) = (*tokenlst)->next;
	}
}

void	collecting_str_loop(t_lst **tokenlst, t_lst **pipeline_tmp)
{
	int	cmd_index;
	int	prev_pos;
...
void	get_pipelinelst(t_lst *tokenlst, t_lst **pipelinelst)
{
	...
	while (tokenlst != NULL)
	{
		...
		if (tokenlst->type == TOKEN_CMD || is_str_token(tokenlst->type))
		{
			...
			collecting_str_loop(&tokenlst, &pipeline_tmp);
			if (*pipelinelst == NULL)
				*pipelinelst = pipeline_tmp;
			else
				get_last(*pipelinelst)->next = pipeline_tmp;
		}
		if (tokenlst && (tokenlst->type != TOKEN_CMD || \
			!is_str_token(tokenlst->type)))
			push_back(pipelinelst, tokenlst->value, \
				tokenlst->type, tokenlst->str_position);
		if (tokenlst)
			tokenlst = tokenlst->next;
	}
}
